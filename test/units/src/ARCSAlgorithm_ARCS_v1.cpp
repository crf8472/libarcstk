 ...
 * \file
 *
 * \brief Unit tests for ARCSAlgorithm<ARCS1>.
 */
...
TEST_CASE ( "ARCSAlgorithm<ARCS1>", "[arcsalgorithm] [calc] [accuraterip]" )
{
	...
	REQUIRE ( instance.types()   == ChecksumtypeSet { type::ARCS1 } );
	...
	SECTION ("Copy construction is correct")
	{
		...
		const ARv1 copied { Copy<ARv1>::construct(instance) };
	...
	SECTION ("Move construction is correct")
	{
		...
		const ARv1 moved { Move<ARv1>::construct(instance) };
	...
	SECTION ("Copy assignment is correct")
	{
		...
		auto copied = ARv1 {};
		Copy<ARv1>::assign(copied, instance);
	...
	SECTION ("Move assignment is correct")
	{
		...
		auto moved = ARv1 {};
		Move<ARv1>::assign(moved, instance);
...
TEST_CASE ( "ARCSAlgorithm<ARCS1> default constructed instance",
		"[arcsalgorithm] [calc] [accuraterip]" )
{
	using ARv1 = arcstk::accuraterip::algorithm::Version1;

	const auto instance = ARv1 {};

	SECTION ( "is ok")
	{
		CHECK ( instance.name() == "AccurateRip ARCSv1" ); // just to call sth
	}
...
TEST_CASE ( "ARCSAlgorithm<ARCS1> property",
		"[arcsalgorithm] [calc] [accuraterip]" )
{
	using ARv1 = arcstk::accuraterip::algorithm::Version1;
...
TEST_CASE ( "ARCSAlgorithm<ARCS1> functions",
		"[arcsalgorithm] [calc] [accuraterip]" )
{
	using arcstk::checksum::type;
	using arcstk::ChecksumtypeSet;
	...
	{
		CHECK ( instance.name() == "AccurateRip ARCSv1" );
	}

	SECTION ("types() is correct")
	{
		CHECK ( instance.types() == ChecksumtypeSet { type::ARCS1 } );
	}
