 ...
 * \file
 *
 * \brief Unit tests for ARCSAlgorithm<ARCS2>.
 */
...
TEST_CASE ( "ARCSAlgorithm<ARCS2>", "[arcsalgorithm] [calc] [accuraterip]" )
{
	...
	REQUIRE ( instance.types()   == ChecksumtypeSet { type::ARCS2 } );
	...
	SECTION ("Copy construction is correct")
	{
		...
		const ARv2 copied { Copy<ARv2>::construct(instance) };
	...
	SECTION ("Move construction is correct")
	{
		...
		const ARv2 moved { Move<ARv2>::construct(instance) };
	...
	SECTION ("Copy assignment is correct")
	{
		...
		auto copied = ARv2 {};
		Copy<ARv2>::assign(copied, instance);
	...
	SECTION ("Move assignment is correct")
	{
		...
		auto moved = ARv2 {};
		Move<ARv2>::assign(moved, instance);
...
TEST_CASE ( "ARCSAlgorithm<ARCS2> default constructed instance",
		"[arcsalgorithm] [calc] [accuraterip]" )
{
	using ARv2 = arcstk::accuraterip::algorithm::Version2;

	const auto instance = ARv2 {};

	SECTION ( "is ok")
	{
		CHECK ( instance.name() == "AccurateRip ARCSv2" ); // just to call sth
	}
...
TEST_CASE ( "ARCSAlgorithm<ARCS2> property",
		"[arcsalgorithm] [calc] [accuraterip]" )
{
	using ARv2 = arcstk::accuraterip::algorithm::Version2;
...
TEST_CASE ( "ARCSAlgorithm<ARCS2> functions",
		"[arcsalgorithm] [calc] [accuraterip]" )
{
	using arcstk::checksum::type;
	using arcstk::ChecksumtypeSet;
	...
	{
		CHECK ( instance.name() == "AccurateRip ARCSv2" );
	}

	SECTION ("types() is correct")
	{
		CHECK ( instance.types() == ChecksumtypeSet { type::ARCS2 } );
	}
