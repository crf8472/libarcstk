...
TEST_CASE ( "MultitrackPartitioner", "[partitioner]" )
{
	...
	const auto c = MultitrackCalcContext(toc);


	using arcstk::details::MultitrackPartitioner;
	const auto mp = MultitrackPartitioner {};

	SECTION ( "Partitioning with TOC in 1 block is correct" )
	{
		const auto  p = mp.create_partitioning(0, 253038 * 588, c);
