...
public:
	...
	using difference_type   = IteratedContainerType::iterator::difference_type;
	 ...
	 * Subtract position
	 */
	friend difference_type operator - (const SampleBlockIterator &lhs,
			const SampleBlockIterator &rhs)
	{
...
template <bool is_const>
auto SampleBlockIterator<is_const>::operator ++ ()
	-> SampleBlockIterator<is_const>&
{
	++it_;
	return *this;
}
...
template <bool is_const>
auto SampleBlockIterator<is_const>::operator ++ (int)
	-> SampleBlockIterator<is_const>
{
...
template <bool is_const>
auto SampleBlockIterator<is_const>::operator + (const uint32_t val)
		-> SampleBlockIterator<is_const>
{
	return SampleBlockIterator(it_ + val);
}
